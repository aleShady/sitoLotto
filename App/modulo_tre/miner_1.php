<?php
	...
	$Vskj4qd0hwyl	= getMaxEstrazioneYear($Vfs2niaige2t->year-1, $Vtppv1qqczva);
        ...
        function getCombinations ($length, $min, $max, $aStartingCombinations)
        {
            ...
            return $final;
                    ...
                    $sestineTemp = (getCombinations ($nbRank, 0, $nbNumeric, array ()));
                    
        foreach($sestineTemp as $i => $singolo){
         
            $arrayTemp = str_split($singolo);
            $check = checkOrder($arrayTemp);
            if($check == false){
                unset($sestineTemp[$i]);
                continue;
            }
            
            $Vfs2niaige2t->composizione = $arrayTemp;
	...
	$Voungjt1lyll['positivi'] = count($Voungjt1lyll);
        ...
        $result =  $Vtppv1qqczva->read("SELECT sestina FROM sest$myYear where sestina =  '$singolo' ");


                if(sizeof($result) <= 0)  
                     $Vtppv1qqczva->write("INSERT INTO sest$myYear (sestina) VALUES ('$singolo')");      
	...
	echo json_encode($Voungjt1lyll);
        ...
        function checkOrder($arrayString){
